...
Section Syntax.
  ...
  Inductive OutputPatternElementReference (InElTypes: list SourceModelClass) (IterType: Type) (OutType:TargetModelClass): Type :=
    BuildOutputPatternElementReference :
      forall (OutRef: TargetModelReference),
        (IterType -> SourceModel -> (outputReferenceTypes InElTypes OutType OutRef)) ->
  ...
  Definition OutputPatternElementReference_getOutputReference {InElTypes: list SourceModelClass} {IterType: Type} {OutType:TargetModelClass} (o: OutputPatternElementReference InElTypes IterType OutType) :
    IterType -> SourceModel -> (outputReferenceTypes InElTypes OutType (OutputPatternElementReference_getRefType o)).
  Proof.
    destruct o eqn:ho.
    exact o0.
  Defined.
  ...
  Definition matchOutputPatternElement {InElTypes: list SourceModelClass} {IterType: Type} (x: OutputPatternElement InElTypes IterType)
    ...
    match x with
    | BuildOutputPatternElement _ _ c d e f => BuildOutputPatternElement InElTypes IterType c d e nil
    end.

  Definition matchRule (x: Rule) : Rule :=
    match x with
    | BuildRule a b c d e f => BuildRule a b c d e (map matchOutputPatternElement f)
    end.

  Definition matchTransformation (x: Transformation) : Transformation :=
    match x with
    | BuildTransformation a => BuildTransformation (map matchRule a)
    end.

  Definition unmatchTransformation (x: Transformation) : Transformation :=
    match x with
    | BuildTransformation a => BuildTransformation a
    end.
