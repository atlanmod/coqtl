...
(* Accessors *)
Definition LocatedElement_getHashCodeId (l : LocatedElement) : string :=
  match l with BuildLocatedElement  hash location  => hash end.
Definition LocatedElement_getLocation (l : LocatedElement) : string :=
  match l with BuildLocatedElement  hash location  => location end.
 
Definition TruthTable_getLocatedElement (t : TruthTable) : LocatedElement :=
  match t with BuildTruthTable locatedelement hash name  => locatedelement end.
Definition TruthTable_getHashCodeId (t : TruthTable) : string :=
  match t with BuildTruthTable locatedelement hash name  => hash end.
Definition TruthTable_getName (t : TruthTable) : string :=
  match t with BuildTruthTable locatedelement hash name  => name end.
 
Definition Port_getLocatedElement (p : Port) : LocatedElement :=
  match p with BuildPort locatedelement hash name  => locatedelement end.
Definition Port_getHashCodeId (p : Port) : string :=
  match p with BuildPort locatedelement hash name  => hash end.
Definition Port_getName (p : Port) : string :=
  match p with BuildPort locatedelement hash name  => name end.
...
Definition Row_getLocatedElement (r : Row) : LocatedElement :=
  match r with BuildRow locatedelement hash  => locatedelement end.
Definition Row_getHashCodeId (r : Row) : string :=
  match r with BuildRow locatedelement hash  => hash end.
 
Definition Cell_getLocatedElement (c : Cell) : LocatedElement :=
  match c with BuildCell locatedelement hash value  => locatedelement end.
Definition Cell_getHashCodeId (c : Cell) : string :=
  match c with BuildCell locatedelement hash value  => hash end.
Definition Cell_getValue (c : Cell) : bool :=
  match c with BuildCell locatedelement hash value  => value end.
...
Fixpoint TTMetamodel_Port_downcastInputPort (po_arg : Port) (l : list TTMetamodel_EObject) : option InputPort := 
  match l with
	 | Build_TTMetamodel_EObject InputPortEClass (BuildInputPort eSuper hash ) :: l' => 
		if beq_Port po_arg eSuper then (Some (BuildInputPort eSuper hash )) else (TTMetamodel_Port_downcastInputPort po_arg l')
...
Fixpoint TTMetamodel_Port_downcastOutputPort (po_arg : Port) (l : list TTMetamodel_EObject) : option OutputPort := 
  match l with
	 | Build_TTMetamodel_EObject OutputPortEClass (BuildOutputPort eSuper hash ) :: l' => 
		if beq_Port po_arg eSuper then (Some (BuildOutputPort eSuper hash )) else (TTMetamodel_Port_downcastOutputPort po_arg l')
...
Fixpoint TTMetamodel_LocatedElement_downcastTruthTable (lo_arg : LocatedElement) (l : list TTMetamodel_EObject) : option TruthTable := 
  match l with
	 | Build_TTMetamodel_EObject TruthTableEClass (BuildTruthTable eSuper hash name ) :: l' => 
		if beq_LocatedElement lo_arg eSuper then (Some (BuildTruthTable eSuper hash name )) else (TTMetamodel_LocatedElement_downcastTruthTable lo_arg l')
...
Fixpoint TTMetamodel_LocatedElement_downcastPort (lo_arg : LocatedElement) (l : list TTMetamodel_EObject) : option Port := 
  match l with
	 | Build_TTMetamodel_EObject PortEClass (BuildPort eSuper hash name ) :: l' => 
		if beq_LocatedElement lo_arg eSuper then (Some (BuildPort eSuper hash name )) else (TTMetamodel_LocatedElement_downcastPort lo_arg l')
...
Fixpoint TTMetamodel_LocatedElement_downcastRow (lo_arg : LocatedElement) (l : list TTMetamodel_EObject) : option Row := 
  match l with
	 | Build_TTMetamodel_EObject RowEClass (BuildRow eSuper hash ) :: l' => 
		if beq_LocatedElement lo_arg eSuper then (Some (BuildRow eSuper hash )) else (TTMetamodel_LocatedElement_downcastRow lo_arg l')
...
Fixpoint TTMetamodel_LocatedElement_downcastCell (lo_arg : LocatedElement) (l : list TTMetamodel_EObject) : option Cell := 
  match l with
	 | Build_TTMetamodel_EObject CellEClass (BuildCell eSuper hash value ) :: l' => 
		if beq_LocatedElement lo_arg eSuper then (Some (BuildCell eSuper hash value )) else (TTMetamodel_LocatedElement_downcastCell lo_arg l')
