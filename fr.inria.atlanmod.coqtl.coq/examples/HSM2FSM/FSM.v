...
Inductive FInitialStateIsFAbstractState : Set :=
   BuildFInitialStateIsFAbstractState :
   FInitialState ->
   FAbstractState ->
   FInitialStateIsFAbstractState.

Inductive FRegularStateIsFAbstractState : Set :=
   BuildFRegularStateIsFAbstractState :
   FRegularState ->
   FAbstractState ->
   FRegularStateIsFAbstractState.


(* Accessors *)
Definition getFStateMachineName (f : FStateMachine) : string :=
  match f with BuildFStateMachine name FStateMachineID  => name end.
Definition getFStateMachineFStateMachineID (f : FStateMachine) : string :=
...
Definition getFTransitionFTransitionID (f : FTransition) : string :=
...
Definition getFAbstractStateFAbstractStateID (f : FAbstractState) : string :=
  match f with BuildFAbstractState name FAbstractStateID  => FAbstractStateID end.
 
Definition getFInitialStateFInitialStateID (f : FInitialState) : string :=
  match f with BuildFInitialState FInitialStateID  => FInitialStateID end.
 
Definition getFRegularStateFRegularStateID (f : FRegularState) : string :=
  match f with BuildFRegularState FRegularStateID  => FRegularStateID end.
...
Definition FSMMetamodel_getEAttributeTypesByEClass (fsec_arg : FSMMetamodel_EClass) : Set :=
  match fsec_arg with
    | FStateMachineEClass => (string * string)
    | FTransitionEClass => (string * string)
    | FAbstractStateEClass => (string * string)
...
| FInitialStateIsFAbstractStateEReference
| FRegularStateIsFAbstractStateEReference
.
...
| FInitialStateIsFAbstractStateEReference => FInitialStateIsFAbstractState
| FRegularStateIsFAbstractStateEReference => FRegularStateIsFAbstractState
  end.
...
| FInitialStateIsFAbstractStateEReference => (FInitialState * FAbstractState)
| FRegularStateIsFAbstractStateEReference => (FRegularState * FAbstractState)
  end.
...
Definition FSMMetamodel_getEObjectFromEAttributeValues (fsec_arg : FSMMetamodel_EClass) : (FSMMetamodel_getEAttributeTypesByEClass fsec_arg) -> FSMMetamodel_EObject :=
  match fsec_arg with
    | FStateMachineEClass => (fun (p: (string * string)) => (Build_FSMMetamodel_EObject FStateMachineEClass (BuildFStateMachine (fst p) (snd p))))
    | FTransitionEClass => (fun (p: (string * string)) => (Build_FSMMetamodel_EObject FTransitionEClass (BuildFTransition (fst p) (snd p))))
    | FAbstractStateEClass => (fun (p: (string * string)) => (Build_FSMMetamodel_EObject FAbstractStateEClass (BuildFAbstractState (fst p) (snd p))))
...
| FInitialStateIsFAbstractStateEReference => (fun (p: (FInitialState * FAbstractState)) => (Build_FSMMetamodel_ELink FInitialStateIsFAbstractStateEReference (BuildFInitialStateIsFAbstractState (fst p) (snd p))))
| FRegularStateIsFAbstractStateEReference => (fun (p: (FRegularState * FAbstractState)) => (Build_FSMMetamodel_ELink FRegularStateIsFAbstractStateEReference (BuildFRegularStateIsFAbstractState (fst p) (snd p))))
  end.
...
Definition beq_FStateMachine (fs_arg1 : FStateMachine) (fs_arg2 : FStateMachine) : bool :=
	...
	( beq_string (getFStateMachineFStateMachineID fs_arg1) (getFStateMachineFStateMachineID fs_arg2) )
			.
...
Definition beq_FTransition (ft_arg1 : FTransition) (ft_arg2 : FTransition) : bool :=
	...
	( beq_string (getFTransitionFTransitionID ft_arg1) (getFTransitionFTransitionID ft_arg2) )
			.
...
Definition beq_FAbstractState (fa_arg1 : FAbstractState) (fa_arg2 : FAbstractState) : bool :=
	...
	( beq_string (getFAbstractStateFAbstractStateID fa_arg1) (getFAbstractStateFAbstractStateID fa_arg2) )
			.
	
Definition beq_FInitialState (fi_arg1 : FInitialState) (fi_arg2 : FInitialState) : bool :=
	( beq_string (getFInitialStateFInitialStateID fi_arg1) (getFInitialStateFInitialStateID fi_arg2) )
			.
	
Definition beq_FRegularState (fr_arg1 : FRegularState) (fr_arg2 : FRegularState) : bool :=
	( beq_string (getFRegularStateFRegularStateID fr_arg1) (getFRegularStateFRegularStateID fr_arg2) )
			.
...
Fixpoint FSMMetamodel_getSuperOfFInitialStateOnLinks (fi_arg : FInitialState) (l : list FSMMetamodel_ELink) : option (FAbstractState) :=
match l with
| (Build_FSMMetamodel_ELink FInitialStateIsFAbstractStateEReference (BuildFInitialStateIsFAbstractState FInitialState_ctr FAbstractState_ctr)) :: l' => 
	  if beq_FInitialState FInitialState_ctr fi_arg then Some FAbstractState_ctr else FSMMetamodel_getSuperOfFInitialStateOnLinks fi_arg l'
| _ :: l' => FSMMetamodel_getSuperOfFInitialStateOnLinks fi_arg l'
| nil => None
end.

Definition getSuperOfFInitialState (fi_arg : FInitialState) (m : FSMModel) : option (FAbstractState) :=
  FSMMetamodel_getSuperOfFInitialStateOnLinks fi_arg (@allModelLinks _ _ m).

Fixpoint FSMMetamodel_getSuperOfFRegularStateOnLinks (fr_arg : FRegularState) (l : list FSMMetamodel_ELink) : option (FAbstractState) :=
match l with
| (Build_FSMMetamodel_ELink FRegularStateIsFAbstractStateEReference (BuildFRegularStateIsFAbstractState FRegularState_ctr FAbstractState_ctr)) :: l' => 
	  if beq_FRegularState FRegularState_ctr fr_arg then Some FAbstractState_ctr else FSMMetamodel_getSuperOfFRegularStateOnLinks fr_arg l'
| _ :: l' => FSMMetamodel_getSuperOfFRegularStateOnLinks fr_arg l'
| nil => None
end.

Definition getSuperOfFRegularState (fr_arg : FRegularState) (m : FSMModel) : option (FAbstractState) :=
  FSMMetamodel_getSuperOfFRegularStateOnLinks fr_arg (@allModelLinks _ _ m).



Fixpoint FSMMetamodel_getSubOfFAbstractStateOnLinks (fa_arg : FAbstractState) (l : list FSMMetamodel_ELink) : option (FSMMetamodel_EObject) := match l with
 						 						 						 			| (Build_FSMMetamodel_ELink FInitialStateIsFAbstractStateEReference (BuildFInitialStateIsFAbstractState FInitialState_ctr FAbstractState_ctr)) :: l' => 
 				  if beq_FAbstractState FAbstractState_ctr fa_arg then Some (FSMMetamodel_toEObjectOfEClass  FInitialStateEClass FInitialState_ctr) else FSMMetamodel_getSubOfFAbstractStateOnLinks fa_arg l'
			 			| (Build_FSMMetamodel_ELink FRegularStateIsFAbstractStateEReference (BuildFRegularStateIsFAbstractState FRegularState_ctr FAbstractState_ctr)) :: l' => 
 				  if beq_FAbstractState FAbstractState_ctr fa_arg then Some (FSMMetamodel_toEObjectOfEClass  FRegularStateEClass FRegularState_ctr) else FSMMetamodel_getSubOfFAbstractStateOnLinks fa_arg l'
			| _ :: l' => FSMMetamodel_getSubOfFAbstractStateOnLinks fa_arg l' 
	| nil => None 
	end.

Definition getSubOfFAbstractState (fa_arg : FAbstractState) (m : FSMModel) : option (FSMMetamodel_EObject) :=
  FSMMetamodel_getSubOfFAbstractStateOnLinks fa_arg (@allModelLinks _ _ m).
...
Definition getFStateMachineTransitions (fs_arg : FStateMachine) (m : FSMModel) : option (list FTransition) :=
...
Definition getFStateMachineStates (fs_arg : FStateMachine) (m : FSMModel) : option (list FAbstractState) :=
...
Definition getFTransitionStateMachine (ft_arg : FTransition) (m : FSMModel) : option (FStateMachine) :=
...
Definition getFTransitionSource (ft_arg : FTransition) (m : FSMModel) : option (FAbstractState) :=
...
Definition getFTransitionTarget (ft_arg : FTransition) (m : FSMModel) : option (FAbstractState) :=
...
Definition getFAbstractStateStateMachine (fa_arg : FAbstractState) (m : FSMModel) : option (FStateMachine) :=
