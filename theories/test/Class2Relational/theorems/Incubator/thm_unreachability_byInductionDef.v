...
Definition reachable_class_step (m: ClassModel) (x y: Class) : Prop :=
  In (ClassMetamodel_toEObject x) (@allModelElements _ _ m) /\
  In (ClassMetamodel_toEObject y) (@allModelElements _ _ m) /\ 
  match (getClassAttributes x m) with
   | Some l =>  exists (attr:Attribute), In (ClassMetamodel_toEObject attr) (@allModelElements _ _ m) /\ 
                                   In attr l /\
...
Definition reachable_table_step (m: RelationalModel) (x y: Table) := 
  In (RelationalMetamodel_toEObject x) (@allModelElements _ _ m) /\ 
  In (RelationalMetamodel_toEObject y) (@allModelElements _ _ m) /\
  match (getTableColumns x m) with
   | Some l => exists (col: Column), In (RelationalMetamodel_toEObject col) (@allModelElements _ _ m) /\ 
                                In col l /\ 
...
Lemma lem_inc_tp_rm:
 (forall (cm : ClassModel) (rm : RelationalModel), rm = execute Class2Relational cm -> (* transformation *)
   forall (a: Attribute),
     In (ClassMetamodel_toEObject a) (@allModelElements _ _ cm) ->
     incl (applyPattern (TransformationA_getRules Class2Relational cm) (ClassMetamodel_toEObject a::nil)) (allModelLinks rm)).
Proof.
